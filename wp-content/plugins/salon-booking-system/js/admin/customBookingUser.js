...
function sln_manageAddNewService($) {
    ...
    $('button[data-collection="addnewserviceline"]').click(function () {
        ...
        var added = false;
        $('.sln-booking-service-line select').each(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.services){
                ...
                if(elems) elems.text(function(){ return $(this).text()+' '+sln_customBookingUser.not_available_string }).parent().css({backgroundColor:'#000',color:'red'});
