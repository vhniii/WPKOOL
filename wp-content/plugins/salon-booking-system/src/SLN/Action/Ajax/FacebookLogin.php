	...
	public function execute()
	{
		if (!isset($this->fbEmail)) {
			if(isset($_POST['fbEmail'])){
				$this->fbEmail = sanitize_text_field(wp_unslash($_POST['fbEmail']));
			}
			if(isset($_POST['fbFirstName'])){
				$this->fbFirstName = sanitize_text_field(wp_unslash($_POST['fbFirstName']));
			}
			if(isset($_POST['fbLastName'])){
				$this->fbLastName = sanitize_text_field(wp_unslash($_POST['fbLastName']));
			}
			if(isset($_POST['fbID'])){
				$this->fbID = sanitize_text_field(wp_unslash($_POST['fbID']));
			}
		}
