...
//                $val = SLN_TimeFunc::evalPickedDate($val);
                ...
                $val = SLN_TimeFunc::evalPickedDate(sanitize_text_field($val));
            }
    ...
    public static function filterUnknownedType($val, &$ret=false){
        ...
        if(is_array($val)){
            ...
            if(!$ret) $ret = array();
            foreach ($val as $k => $v) {
                $ret[$k] =is_array($v) ? self::filterUnknownedType($v,$ret[$v]) :(is_numeric($v) || is_bool($v) || empty($v) ? $v : sanitize_text_field(wp_unslash($v)));
            }

            return $ret;
        }
        return is_numeric($val) || is_bool($val) || empty($val) ? $val : sanitize_text_field($val);
    }
