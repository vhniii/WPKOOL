...
class WBK_Service_Schedule {
	...
	public function buildSchedule( $day, $ignore_optimization = false, $skip_gg_calendar = false ){
		...
		if( get_option( 'wbk_multi_booking', 'disabled' ) == 'disabled' ){
			...
			if( $this->service->getStep() > $this->service->getDuration() ){
				$step = $this->service->getStep() * 60;			
			} else {
	...
	public function getDayStatus( $day ){
		...
		if( is_array( $whole_day_checkin ) ){
			if( in_array( $this->service_id, $whole_day_checkin ) ){
				if( WBK_Db_Utils::getNumberOfAppointmentsOnDay( $this->service_id, $day ) > 0 ){
					return 0;
				}
			}
		}
			...
			for ( $i =  0; $i < count( $tomorrows_time_slots ); $i++ ) {
				...
				if( is_array( $add_before_after ) ){
					if( in_array( $this->service_id, $add_before_after ) ){
						if( $i > 0 ){
							$prev_slot = $tomorrows_time_slots[$i - 1];
							if( $prev_slot->getStatus() > 0 || is_array( $prev_slot->getStatus() ) ){
								$tomorrows_time_slots[$i]->setStatus( -2 );
							}
						}
						if( $i < ( count( $tomorrows_time_slots ) - 1 ) ){
							$next_slot = $tomorrows_time_slots[$i +1];
			...
			if ( $timeslot->getStatus() == 0 || is_array( $timeslot->getStatus() ) ) {
				if ( $timeslot->getStart() >= $time_after ) {
					...
					if( get_option( 'wbk_allow_ongoing_time_slot', 'disallow' ) == 'disallow' ){
						if( $timeslot->getStart() > ( time() + $this->service->getPrepareTime() * 60 * 60  ) ){
							$ongoing_valid = true;
						} 
					} else {
	...
	public function getAvailableCount( $time, $ignore_parital = false  ){
		...
		if( $service->getQuantity() > 1 && get_option( 'wbk_gg_2way_group', 'lock' ) == 'reduce' ){
