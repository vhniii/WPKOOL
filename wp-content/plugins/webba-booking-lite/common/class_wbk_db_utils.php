...
class WBK_Db_Utils {
	...
	static function deleteExpiredAppointments(){
		...
		if( get_option( 'wbk_gdrp', 'disabled' ) == 'enabled' ){
			...
			$wpdb->query( $wpdb->prepare( "DELETE FROM wbk_cancelled_appointments WHERE time < %d", $time ) );
		}
	}
	...
	public static function message_placeholder_processing( $message, $appointment, $service, $total_amount = null, $current_category = 0, $multi_token = null, $multi_token_admin = null ){	
		...
		// end total amount
		$price_format = get_option( 'wbk_payment_price_format', '$#price' );
		$tax_amount =  str_replace( '#price', number_format( $tax_amount,  get_option( 'wbk_price_fractional', '2' ), '.', ''  ), $price_format );
		$subtotal_amount =  str_replace( '#price', number_format( $subtotal_amount,  get_option( 'wbk_price_fractional', '2' ) ), $price_format );
		// beging extra data
		...
		$price_format = get_option( 'wbk_payment_price_format', '$#price' );
		$one_slot_price = str_replace( '#price', number_format( $service->getPrice(),  get_option( 'wbk_price_fractional', '2' ), '.', ''  ), $price_format );
	...
	static function getCountOfAppointmentsByEmailService( $email, $service_id ){
		global $wpdb;
		$count = $wpdb->get_var( $wpdb->prepare( " SELECT COUNT(*) FROM wbk_appointments WHERE email=%s and service_id=%d and time > %d", $email, $service_id, time() ) );
		return $count;
	} 
	...
	static function copyAppointmentToCancelled( $appointment_id, $cancelled_by ){
		...
		$wpdb->insert( 
			...
			array( 
				...
				'paid_amount' => $appointment_data['paid_amount'], 
	...
	static function getPaymentFields(){
		return array(
			...
			'address_line1' => __( 'Address line 1', 'wbk' ),
			'address_line2' => __( 'Address line 2', 'wbk' ),
			'address_city' => __( 'City', 'wbk' ),
			'address_state' => __( 'State', 'wbk' ),
			'address_zip' => __( 'ZIP', 'wbk' ),
			'address_country' => __( 'Country', 'wbk' )
	...
	static function getNumberOfAppointmentsOnDay( $service_id, $day ){
		global $wpdb;
		$arrIds = array();
		$autolock_mode = get_option( 'wbk_appointments_auto_lock_mode', 'all' );
		if( $autolock_mode == 'all' ){
