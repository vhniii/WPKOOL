...
class WBK_Ajax_Controller {
			...
			$date_format = WBK_Date_Time_Utils::getDateFormat();
	 		...
	 		while ( $i <= $output_count ) {
	 			...
	 			if ( $day_status == 1 ) {
					...
					$date_regular = date_i18n ( $date_format, $day_to_render );
					$timezone = new DateTimeZone( get_option( 'wbk_timezone' ) );
					$current_offset =  $offset * - 60 - $timezone->getOffset( new DateTime );
					$date_local = date_i18n ( $date_format, $day_to_render + $current_offset );
					...
					$day_title = str_replace( '#local_date', $date_local, $day_title );
		 			...
		 			if ( substr_count( $day_slots, 'wbk-timeslot-btn' ) == 1  &&  get_option( 'wbk_skip_timeslot_select', 'disabled' ) == 'enabled' ){
	...
	public function	ajaxCancelAppointment(){
		...
		foreach( $appointment_ids as $appointment_id ){	
            ...
            WBK_Db_Utils::copyAppointmentToCancelled( $appointment_id, __( 'Customer', 'wbk' ) );
	...
	public function ajaxPrepareServiceData(){
		...
		if( !is_array( $service_id ) ){			
	 		...
	 		if ( get_option( 'wbk_date_input', 'popup' ) == 'popup' || get_option( 'wbk_date_input', 'popup' ) == 'classic' ){
				$result['disabilities'] = WBK_Date_Time_Utils::getServiceAbiliy( $service_id );
	...
	protected function render_booking_form_multiple( $service_ids, $times, $offset = 0 ){
		...
		$form_label_initial =  explode( '[split]', get_option( 'wbk_form_label', '' ) );
		...
		if ( $form == 0 ){
			...
			$html .= '<input name="wbk-name" autocomplete="disabled" type="text" class="wbk-input wbk-width-100 wbk-mb-10" id="wbk-customer_name" />';
			$html .= '<label class="wbk-input-label" for="wbk-customer_email">' . $email_label . '</label>';
			$html .= '<input name="wbk-email" autocomplete="disabled" type="text" class="wbk-input wbk-width-100 wbk-mb-10" id="wbk-customer_email" />';
			$html .= '<label class="wbk-input-label" for="wbk-customer_phone">' . $phone_label . '</label>';
			$html .= '<input name="wbk-phone" autocomplete="disabled" autocomplete="disabled" type="text" class="wbk-input wbk-width-100 wbk-mb-10" id="wbk-customer_phone" />';
	...
	protected function render_booking_form( $service_id, $time, $offset = 0 ){
		...
		if ( $form == 0 ){
			...
			$html .= '<input name="wbk-email" autocomplete="disabled" type="text" class="wbk-input wbk-width-100 wbk-mb-10" id="wbk-customer_email" />';
			$html .= '<label class="wbk-input-label" for="wbk-customer_phone">' . $phone_label . '</label>';
			$html .= '<input name="wbk-phone" autocomplete="disabled" type="text" class="wbk-input wbk-width-100 wbk-mb-10" id="wbk-customer_phone" />';
			$html .= '<label class="wbk-input-label" for="wbk-customer_desc">' . $comment_label . '</label>';
	 		$html .= '<textarea name="wbk-comment" rows="3" class="wbk-input wbk-textarea wbk-width-100 wbk-mb-10" id="wbk-customer_desc"></textarea> ';
		} else {
			$cf7_form = do_shortcode( '[contact-form-7 id="' . $form . '"]' );
...
function wbk_set_appointment_as_paid_with_coupon( $app_ids ){
	...
	foreach ( $app_ids as $app_id ){
		WBK_Db_Utils::setPaymentMethodToAppointment( $app_id, '100% discount used' );
	}
	if( count( $app_ids ) > 0 ){
