...
function wbk_renderSetDate( scroll ) {
    ...
    wbkjQ.post( wbkl10n.ajaxurl, data, function(response) {   
		...
		if( wbkl10n.date_input == 'popup' || wbkl10n.date_input == 'classic'  ){
			...
			if ( wbkl10n.mode == 'extended' ){
				wbkjQ('#wbk-date-container').html( sep_html + '<div class="wbk-col-12-12"><label class="wbk-input-label">' + wbkl10n.selectdatestart + '</label><input value="' + wbkl10n.selectdate + '" type="text" class="wbk-input"  ' + prefil_date + ' id="wbk-date" /></div>');
			} else {
				wbkjQ('#wbk-date-container').html(  sep_html + '<div class="wbk-col-12-12"><label class="wbk-input-label">' + wbkl10n.selectdatestartbasic + '</label><input value="' + wbkl10n.selectdate + '" type="text" ' + prefil_date + ' class="wbk-input" id="wbk-date" /></div>');
		 	}
		 	if( prefil_date != ''){
			if ( wbkl10n.mode == 'extended' ){
	 				wbk_clearSetTime();
...
function wbk_searchTime() {
    ...
    wbkjQ.post( wbkl10n.ajaxurl, data, function(response) {   
    	...
    	if ( response == 0 || response == -1 || response == -2 || response == -3 || response == -4 || response == -4 || response == -6 || response == -7){
    		...
    		wbkjQ(wbkl10n.scroll_container).animate({ scrollTop: wbkjQ('#wbk-slots-container').offset().top - 120 }, 1000);
    	...
    	if (response == 0 || response == -1){
    		...
    		response_obj = wbkjQ.parseJSON( response );
     		if( response_obj.dest == 'slot' ){
      		...
      		wbkjQ(wbkl10n.scroll_container).animate({ scrollTop: wbkjQ('.wbk-more-container').last().offset().top - 120 }, 1000);
		...
		if( wbkl10n.multi_booking == 'enabled' || wbkl10n.multi_booking == 'enabled_slot' ){
			...
			if( selected_cnt > 0 ){
				...
				wbkjQ('#wbk-to-checkout').click(function() {
					...
					wbkjQ(wbkl10n.scroll_container).animate({ scrollTop: wbkjQ('#wbk-booking-form-container').last().offset().top - 120 }, 1000);
...
function wbk_set_payment_events(){
	wbkjQ('.wbk-payment-init').click(function() {
		...
		wbkjQ(wbkl10n.scroll_container).animate({ scrollTop: wbkjQ('#wbk-payment').last().offset().top - 120 }, 1000);
	 	...
	 	wbkjQ.post( wbkl10n.ajaxurl, data, function(response) {
            wbkjQ('#wbk-payment').fadeOut('fast', function(){
            	if( method == 'woocommerce' ){					 
					...
					wbkjQ('#wbk-payment').html( 'WooCommerce integration is available in the Premium version' );
