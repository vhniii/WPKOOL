...
function getAllowedClassNames() {
	return array( 'WBK_Appointments_Table' , 'WBK_Email_Templates_Table', 'WBK_Service_Categories_Table', 'WBK_Service_Categories_Table', 'WBK_GG_Calendar_Table', 'WBK_Coupons_Table', 'WBK_Cancelled_Appointments_Table' );
}
			...
			if( $field->name == 'extra' ){
				...
				if( $ids != ''  ){
					...
					foreach ( $ids as $current_custom_id ) {
						// get field title by ID
						$col_title = '';
						$extra_data = $wpdb->get_results( 'SELECT extra from wbk_appointments where extra like "%' . $current_custom_id . '%"', ARRAY_N );
						if( is_array( $extra_data ) ){
							if( isset( $extra_data[0] ) ){
								$extra_data = json_decode( $extra_data[0][0] );
								if( is_array( $extra_data ) ){
									foreach( $extra_data as $field_data ){
										if( $field_data[0] == $current_custom_id ){
