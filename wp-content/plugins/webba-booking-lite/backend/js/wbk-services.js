...
function add_service() {
	...
	jQuery( '#wbk-service-min-quantity' ).val('1');

	jQuery( '#wbk-service-priority' ).val('0');
	jQuery( '#wbk-service-quantity' ).prop('disabled', false);
	jQuery( '#wbk-service-min-quantity' ).prop('disabled', false);
        ...
        buttons: [
        	{
        		...
        		click: function() {
						...
						if ( !wbkCheckIntegerMinMax( step, 1, 1440 ) ){
							error_status = 1;
			 				jQuery( '#wbk-service-step' ).addClass('wbk-input-error');	 				
						}
						...
						var min_quantity = jQuery.trim( jQuery( '#wbk-service-min-quantity' ).val() );
						if ( !wbkCheckIntegerMinMax( min_quantity, 1, quantity) ){
						 	error_status = 1;
			 			 	jQuery( '#wbk-service-min-quantity' ).addClass('wbk-input-error');	 				
						}
			...
			var data = {
	 		...
	 		jQuery.post(ajaxurl, data, function(response) {	
					...
					if ( matches ) {
						...
						jQuery( '#wbk-service-min-quantity' ).val( objdata.min_quantity );
...
function setQuantityOnChange() {
	jQuery( '#wbk-service-quantity' ).focusout(function() {
 	...
 	jQuery( '#wbk-service-min-quantity' ).focusout(function() {
		var min_quantity = jQuery( '#wbk-service-min-quantity' ).val();
		var quantity = jQuery( '#wbk-service-quantity' ).val();
		var error_status = 0;
 
		if ( !wbkCheckIntegerMinMax(min_quantity,1,quantity ) ){
			jQuery( '#wbk-service-min-quantity' ).addClass('wbk-input-error');
			jQuery( '#wbk-service-min-quantity' ).focus();
			error_status =1;
		} else {
			jQuery( '#wbk-service-min-quantity' ).removeClass('wbk-input-error');			
		}
		if ( error_status == 0 ){
		}
 	});
