...
jQuery(function ($) {
  ...
  var CloudAppButton = window.opener.document.getElementsByClassName( 'window-id-' + window.self.name );

  var selected = [];

  // Get button attributes.
  var _cardinality = $( CloudAppButton ).data( 'cardinality' );
  var _multiselect = ( _cardinality > 1 ) ? true : false;
  var _disable_select = false;
  $( 'a#pick' ).addClass( 'disabled' );

  // Select photos.
  $( '#media-items a.item' ).on( 'click', function( e ) {
    ...
    if ( !$( this ).hasClass( 'disabled' ) ) {
      if ( selected.length ) {
        ...
        for ( var i = 0; i < selected.length; i++ ) {
          var _item = $( '#media-items a[data-index="' + selected[i] + '"]' ).data( 'standard-img' );
          _item += '::' + $( '#media-items a[data-index="' + selected[i] + '"]' ).data( 'link' );
          _files.push( _item );
        }
  ...
  // Double click.
  $( '#media-items a.item' ).on( 'dblclick', function( e ) {
    if ( _disable_select ) {
