    ...
    if (!class_exists('DOPBSPFrontEndCalendar')){
        class DOPBSPFrontEndCalendar extends DOPBSPFrontEnd{
            ...
            function display($atts){
                ...
                $refferal_id = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$DOPBSP->tables->settings.' WHERE name=%s',
                                                            'refferal_id'));
                $refferal_display = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$DOPBSP->tables->settings.' WHERE name=%s',
                                                            'refferal_display'));
...
// HOOK (dopbsp_frontend_content_after_calendar) ******************************* Add content after calendar.
                ...
                if(isset($refferal_display) && $refferal_display->value === 'true') {
                         array_push($html, '      <div class="DOPBSPCalendar-refferal">Powered by <a href="https://pinpoint.world/wordpress-booking?ref='.(isset($refferal_id->value) ? $refferal_id->value : 0).'" target="_blank">PINPOINT.WORLD</a></div>');
                }
                return implode("\n", $html);
            }
